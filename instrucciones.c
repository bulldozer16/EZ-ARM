...
/* Archivos que tendrán los contenidos del banco
   de registros y memoria de programa */
...
void print_flags()
{
	flags = fopen(".flags.txt", "w");
	printf("Negative: %d\nZero: %d\nCarry: %d\nOverflow: %d\n", n, z, c, v);
	fprintf(flags, "N: %d, Z: %d, C: %d, V: %d", n, z, c, v);
	fclose(flags);
}
...
int str(int cond, int rd, int rn, int src2, int mode) 
{
	int exec = eval_cond(cond);
	if (exec)
	{
...
int ldr(int cond, int rd, int rn, int src2, int mode) 
{
	int exec = eval_cond(cond);
	if (exec)
	{
...
int strb(int cond, int rd, int rn, int src2, int mode) 
{
	int exec = eval_cond(cond);
	if (exec)
	{
...
int ldrb(int cond, int rd, int rn, int src2, int mode) 
{
	int exec = eval_cond(cond);
	if (exec)
	{
...
void load_data_mem(){
	FILE* data_mem = fopen("Memoria_datos.txt" , "r");
	for (int i = 256; i < 512; i++){
		char buffer[20];
		char* p;
		int ival;
		if(fgets (buffer, 20, data_mem) != NULL ){
			strtol(buffer, &p, 0);
			ival = (int)strtol(p, NULL, 0);
			store_memory(i, ival);
		}	
	}	
}
